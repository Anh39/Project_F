...
model = causal_lora_model(folder_path.model.gemma,folder_path.output.gemma.path,lam=0,
                          data_threshold=2,seed=seed,max_data_remember=5)

causal_mmlu.custom_init()
ds = causal_mmlu.get_data(amount=10000,seed=seed)
train_data,test_data = train_test_split(ds,test_size=0.1,random_state=seed)
evaluator = causal_mmlu_eval(model,train_dataset=train_data,eval_dataset=test_data,step=450,
                             use_selector=False,fixed_adapter='single')
evaluator.lr = 0.0001
result = evaluator.evaluate_and_train(start_from=0,end_to=9000)
